...
if(!empty($jwt)){
    ...
    $sql_gr1 = "SELECT ('gr1') AS key_p, search, SUM(nn) as nn, SUM(pcs) as pcs FROM(
    ...
    while($row1 = $stmt_gr1->fetch(PDO::FETCH_ASSOC)) { //===== กลุ่ม 1  
        $ss_gr1 = $row1['search'];
        $sql = "SELECT ('data') AS key_p, search, code, spec, SUM(nn) as nn, SUM(pcs) AS pcs FROM(
            SELECT date_rec,bill_rec,receive.code,CONCAT(dia,' ',color,' ',knot,' ',ms,ms_unit,'x',md,md_unit,'x',ml,ml_unit,' ',label) AS spec, search,1 AS nn, receive.pcs FROM receive INNER JOIN product ON receive.code = product.code  WHERE ((date_rec BETWEEN '${datefm}' AND '${dateto}') ${depart_rec} ${depart_post} AND CONCAT('#',bill_rec,' ',receive.code,' ',dia,' ',color,' ',knot,' ',ms,ms_unit,'x',md,md_unit,'x',ml,ml_unit,' ',label,' ',search) LIKE '%${search}%' AND search = '${ss_gr1}') 
                ) AS T_gr1 GROUP BY code ORDER BY code ASC";
