...
<script> 
  window.onload = function(){
    ...
    $.ajax({
      ...
      success: function(result){
        ...
        $("#head_rep").html("รายงานรับเข้า (แยกรายการ)");
  ...
  function show_hTable(x){  //========= ฟังก์ชั่นเพิ่ม หัวตาราง    
      ...
      row.innerHTML = `
        ...
        <th class="text-center tb_bold">วันรับ</th>
        <th class="text-center tb_bold">เลขบิล</th>
  ...
  function show_dataTable(x,yy,ob){  //========= ฟังก์ชั่นเพิ่ม Row ตาราง    
    ...
    let n_col = 6; //==== จำนวน Column ที่แสดงทั้งหมด
    let n_col_span = 5; //==== จำนวน Column ที่ผสานเซลกันบรรทัดสรุปข้อมูล
    let col = [];
    let nn = 0;
    let grouprow = '';
    let line_t = '';
    ...
    if(ob.key_p == 'gr1'){
      n_col--;
      grouprow = `<div class="tb_bold_2" align="center">รวม ${ob.code} &nbsp;&nbsp;( ${ob.nn} รายการ )</div>`;
    }else if(ob.key_p == 'gr2'){
      n_col--;
      grouprow = `<div class="tb_bold_2" align="center" style="background-color: #d7d7d7; font-weight: bold;">รวม ${ob.search} &nbsp;&nbsp;( ${ob.nn} รายการ )</div>`;
      
    }else if(ob.key_p == 'all'){
      n_col--;
      grouprow = `<div class="tb_bold_2" align="center" style="background-color: #d7d7d7; font-weight: bold;">รวมทั้งหมด &nbsp;&nbsp;( ${ob.nn} รายการ )</div>`;
    }    
    ...
    if(ob.key_p == 'data'){ //=========== แสดงข้อมูลในตาราง ==================================
      ...
      col[1].innerHTML = `<div class="nm" align="center">`+to_dmY(ob.date_rec)+`</div>`;
      col[2].innerHTML = `<div class="nm" align="center">`+ob.bill_rec+`</div>`;
      col[3].innerHTML = `<div class="nm" align="center">${ob.code}</div>`;
      col[4].innerHTML = `<div class="nm" align="left">`+ob.spec+`</div>`;
      ...
      col[nn].setAttribute('colspan',n_col_span);
      col[nn].innerHTML = grouprow;   

    } 
      col[nn+1].innerHTML = `<div `+line_t+` align="center">`+addCommas((ob.pcs*1).toFixed(0))+`</div>`;   
} //=========================================================================================
